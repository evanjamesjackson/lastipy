...
class RecentArtistsTest(unittest.TestCase):
    ...
    def test_fetch_multiple_pages(self, mock_requests_get):
        ...
        mock_responses[0].json.return_value = self._build_artist_json_response(
            'Bee Gees', '5', '3')
        mock_responses[1].ok = True
        mock_responses[1].json.return_value = self._build_artist_json_response(
            'The Beatles', '10', '3')
        mock_responses[2].ok = True
        mock_responses[2].json.return_value = self._build_artist_json_response(
            'Cream', '15', '3')
    ...
    def test_fetch_fails_after_retries(self, mock_requests_get):
        ...
        mock_response = Mock()
        mock_response.ok = False
        mock_response.json.return_value = self._build_artist_json_response(
            'Cream', '15', '1')
