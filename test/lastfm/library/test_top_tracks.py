...
class TopTracksFetcherTest(unittest.TestCase):
    ...
    def test_one_page_of_results(self, mock_paginated_endpoint_fetch):
        ...
        expected_track_2 = ScrobbledTrack(
        ...
        mock_paginated_endpoint_fetch.return_value = [
            json_response_page_1, json_response_page_2]

        fetched_tracks = fetch_top_tracks(user=self.dummy_user,
                                          api_key=self.dummy_api_key, a_period=period.ONE_MONTH)
    ...
    #     expected_track_1 = ScrobbledTrack(
    #         track_name="Penny Lane", artist="The Beatles", playcount=5)
    #     expected_track_2 = ScrobbledTrack(
    #         track_name="Won't Get Fooled Again", artist="The Who", playcount=6)
    #     expected_track_3 = ScrobbledTrack(
    #         track_name="Like the FBI", artist="Bob Dylan", playcount=10)
    #     expected_tracks = [expected_track_1,
    #                        expected_track_2, expected_track_3]
    ...
    #         api_key='', user="sonofjack3", a_period=period.SEVEN_DAYS)
    #     self.assertCountEqual(fetched_tracks, expected_tracks)

    # @ patch('requests.get')
    # def test_songs_with_one_playcount_ignored(self, mock_requests_get):
    #     ignored_track_1 = ScrobbledTrack(
    #         track_name="Stayin' Alive", artist="Bee Gees", playcount=1)
    #     non_ignored_track = ScrobbledTrack(
    #         track_name="Ventura Highway", artist="America", playcount=5)
    #     ignored_track_2 = ScrobbledTrack(
    #         track_name="Anesthetized Lesson", artist="Gum", playcount=1)
    ...
    #         fetch_top_tracks(api_key='', user='sonofjack3',
    #                          a_period=period.SEVEN_DAYS)
