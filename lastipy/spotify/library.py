...
def get_saved_tracks(spotify):
    ...
    keep_fetching = True
    while keep_fetching:
        json_response = spotify.current_user_saved_tracks(
...
def add_tracks_to_library(spotify, tracks):
    ...
    for chunk in track_chunks:
        spotify.current_user_saved_tracks_add(
            [track.spotify_id for track in chunk])
...
def add_albums_to_library(spotify, albums):
    ...
    for chunk in album_chunks:
        spotify.current_user_saved_albums_add(
            [album.spotify_id for album in chunk])
